...
//            params.add(0, "LD_LIBRARY_PATH=" + PrefStore.getLibsDir(c) + ":$LD_LIBRARY_PATH");
    ...
    static boolean updateEnv(final Context c) {
        // stop services
        execServices(c, new String[]{"telnetd", "httpd", "pulseaudio"}, "stop");
        ...
        // extract libs assets
        if (!extractDir(c, PrefStore.getLibsDir(c), "libs/" + PrefStore.getArch(), ""))
            return false;
        ...
        // start services
        execServices(c, new String[]{"telnetd", "httpd", "pulseaudio"}, "start");
    ...
    static boolean removeEnv(Context c) {
        ...
        File libsDir = new File(PrefStore.getLibsDir(c));
        cleanDirectory(libsDir);
    ...
    /**
     * Start/stop pulseaudio daemon
     *
     * @param c   context
     * @param cmd command: start, stop or restart
     * @return true if success
     */
    static boolean pulseaudio(Context c, String cmd) {
        List<String> params = new ArrayList<>();
        if (cmd == null) cmd = PrefStore.isPulseAudio(c) ? "start" : "stop";
        switch (cmd) {
            case "restart":
            case "stop":
                params.add("pkill -9 pulseaudio");
                if (cmd.equals("stop")) break;
            case "start":
                if (!PrefStore.isPulseAudio(c)) break;
                makePulseAudioConf(c);
                String args = "-D -n --exit-idle-time=-1 --disallow-exit=true --disable-shm=true --use-pid-file=false --log-target=stderr";
                args += " --dl-search-path=" + PrefStore.getLibsDir(c);
                args += " -F " + PrefStore.getEtcDir(c) + "/pulseaudio.conf";
                params.add("pgrep pulseaudio >/dev/null && exit");
                params.add("export LD_LIBRARY_PATH=\"" + PrefStore.getLibsDir(c) + ":$LD_LIBRARY_PATH\"");
                params.add("export TMPDIR=\"" + PrefStore.getTmpDir(c) + "\"");
                params.add("export HOME=\"$TMPDIR\"");
                params.add("cd \"$HOME\"");
                params.add("pulseaudio " + args);
        }
        return params.size() > 0 && exec(c, "sh", params);
    }

    /**
     * Make pulseaudio.conf file
     *
     * @param c context
     * @return true if success
     */
    private static boolean makePulseAudioConf(Context c) {
        boolean result = false;
        BufferedWriter bw = null;
        try {
            String f = PrefStore.getEtcDir(c) + "/pulseaudio.conf";
            bw = new BufferedWriter(new FileWriter(f));
            bw.write("load-module module-native-protocol-tcp auth-anonymous=true port=" + PrefStore.getPulseAudioPort(c) + "\n");
            bw.write("load-module module-sles-sink\n");
            result = true;
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
